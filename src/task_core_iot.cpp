...
String rpc_req_topic =
    String("iot/gateway/") + DEVICE_NAME + "/rpc/request/+/+";
String rpc_resp_prefix =
    String("iot/gateway/") + DEVICE_NAME + "/rpc/response/";
...
void mqtt_callback(char *topic, byte *payload, unsigned int length) {
  ...
  if (topicStr.indexOf("/rpc/request/") != -1) {
    ...
    if (method == "setLED") {
      ...
      mqttClient.publish((rpc_resp_prefix + rpc_id).c_str(),
                         newState ? "1" : "0");
...
void task_core_iot_run(void *pvParameters) {
  ...
  while (1) {
    if (WiFi.status() == WL_CONNECTED && !CORE_IOT_SERVER.isEmpty()) {
      ...
      if (mqttClient.connected() &&
          xQueueReceive(xQueueMLToServer, &telemetry_data, pdMS_TO_TICKS(10)) ==
              pdTRUE) {
