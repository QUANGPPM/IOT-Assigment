...
bool setupTinyML() {
  ...
  model = tflite::GetModel(
      dht_anomaly_model_tflite); // g_model_data is from model_data.h
  if (model->version() != TFLITE_SCHEMA_VERSION) {
    error_reporter->Report("Model provided is schema version %d, not equal to "
                           "supported version %d.",
...
void tiny_ml_task(void *pvParameters) {
  ...
  while (1) {
    ...
    if (xQueueReceive(xQueueSensorToML, &sensor_data, portMAX_DELAY) ==
        pdTRUE) {
      ...
      if (sensor_data.temperature <= -999.0f) {
        ...
        processed_data.temperature = sensor_data.temperature;
        processed_data.humidity = sensor_data.humidity;
        // Scale score về thang 100 (0-100)
        processed_data.anomaly_score = output->data.f[0] * 100.0f;

        // Stage 3.1: Phân loại trạng thái dựa vào anomaly_score
        // Đọc ngưỡng trong vùng bảo vệ mutex
        float warn_thr, danger_thr;
        if (xSemaphoreTake(xMutexThresholds, pdMS_TO_TICKS(10)) == pdTRUE) {
          warn_thr = g_anomaly_warn;
          danger_thr = g_anomaly_danger;
        ...
        if (processed_data.anomaly_score >= danger_thr) {
          processed_data.status = STATUS_DANGER;
        } else if (processed_data.anomaly_score >= warn_thr) {
          processed_data.status = STATUS_WARNING;
        } else {
          processed_data.status = STATUS_NORMAL;
        }

        Serial.printf("[ML] Score: %.4f | Thres: W=%.1f D=%.1f | Status: %d\n",
                      processed_data.anomaly_score, warn_thr, danger_thr,
                      (int)processed_data.status);
      }
      ...
      xQueueOverwrite(xQueueMLToWeb,
                      &processed_data); // Gửi dữ liệu cho Web Server
    }
  }
}